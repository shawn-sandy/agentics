...
- **claude.ai sync staleness check** — `scripts/check-claude-ai-sync.mjs` diffs each plugin's skills and commands against the frozen claude.ai snapshot Claude Desktop actually runs (`~/Library/Application Support/Claude/local-agent-mode-sessions/<a>/<b>/rpm/`), and warns when the Desktop copy is behind the repo or still carries a plugin removed from `marketplace.json`. Runs on demand, and quietly from a `SessionStart` hook that prints only when something has drifted. Advisory and always exit 0 — the remedy is refreshing the marketplace on claude.ai, not a repo change
