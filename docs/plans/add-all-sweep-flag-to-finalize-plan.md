...
- [x] Sweep discovery matches only files with a `plan-status` meta tag valued `todo` or `in-progress` (non-plan HTML is never a candidate), excludes `index.html`, and never descends into `archive/`.
- [x] Candidates are scored with the cheap token-evidence pass only, non-interactively (token-less plans score 0% rather than prompting); per-criterion verification and the objective test run solely on user-selected plans.
...
- Report-only background agent for scheduled sweeps:
  ...
  that sweeps the plans directory for done-but-unmarked plans (grep -l for a
  <meta name="plan-status"> tag valued todo or in-progress, excluding
  index.html and archive/), runs the cheap token-evidence scan from the finalize-plan skill's
