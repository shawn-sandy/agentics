...
- kit/plugins/memory-tools/skills/insights-review/references/already-applied.md (new) — the extractable three-status already-applied detector
...
1. Scaffold kit/plugins/memory-tools/skills/insights-review/SKILL.md with `name: insights-review`, a three-part `description` of 200 characters or fewer, an `allowed-tools` list covering AskUserQuestion, Bash(python3 *), Glob, Grep, Read, Write, Edit and Skill, and the repo's plan-mode guard line verbatim as its first step Why: the skill writes files, and this repo enforces the guard string and the description budget with tests that fail the build if either is missing Verify: `python3 tests/plugins/measure_description_budget.py` reports no violation for the new file and `bash tests/plugins/test-exitplanmode-guard.sh` exits 0.
...
3. Write references/already-applied.md shipping an extractable `python3` block that compares each `memory` record's heading against the target file's headings and then its bullet text with whitespace and punctuation normalised, emitting one of three statuses — `applied` when heading and bullets both match, `applied-drifted` when the heading matches but the wording has changed, `new` when the heading is absent Why: three of five suggestions in a real report are already applied, and a reworded suggestion matched on heading alone would otherwise be skipped silently and its improvement lost Verify: run the extracted block over the fixture pair and confirm the split is 2 `applied`, 1 `applied-drifted`, 2 `new`.
...
- [ ] The shipped detector splits the 5 fixture suggestions into 2 `applied`, 1 `applied-drifted`, and 2 `new`
...
- [ ] `python3 tests/plugins/measure_description_budget.py` reports no violation for the new SKILL.md
...
- Unit: three-status detection. File: tests/plugins/test-insights-review.sh; Targets: the extractable python3 block in references/already-applied.md; Key cases: heading and bullets both match yields applied, heading matches with reworded bullets yields applied-drifted, heading absent yields new, bullet whitespace and punctuation differences alone do not count as drift
...
Finally run the repo guards that the new files must not break: `python3 tests/plugins/measure_description_budget.py`, `bash tests/plugins/test-exitplanmode-guard.sh`, `BASE_REF=main node scripts/check-plugin-versions.mjs`, and `node scripts/build-readme-table.mjs --check`. All four must exit 0.
