...
3. **Define the generated-doc template** (embedded in SKILL.md Step 8 guidance)
   ...
   - **CHANGELOG handling**: the "What shipped" section must cite the plugin CHANGELOG entry range rather than reproduce it verbatim. Example footer: *"See [plan-interview CHANGELOG v1.13.0](../kit/plugins/plan-interview/CHANGELOG.md#1130) for the authoritative feature list."* This avoids drift between the CHANGELOG and the generated doc.
