...
6. Add the same one-line boundary statement to kit/plugins/social-media-tools/README.md pointing readers from write-guide toward teach-artifact when they want a shareable page, add the matching 2.22.1 entry to kit/plugins/social-media-tools/CHANGELOG.md, and raise social-media-tools from 2.22.0 to 2.22.1 in .claude-plugin/marketplace.json. Why: the boundary only works if it is stated on both sides; any edit inside a plugin directory requires that plugin's own version bump or the version guard fails the pull request; and the marketplace rules require a changelog entry for every plugin whose version moves, not only the one gaining the feature, so a bump without an entry ships a release with no history. Verify: `git fetch origin && BASE_REF=main node scripts/check-plugin-versions.mjs` exits 0 and reports both plugins above their base-branch versions, and `grep -q '2.22.1' kit/plugins/social-media-tools/CHANGELOG.md` succeeds.
...
- [ ] `git fetch origin && BASE_REF=main node scripts/check-plugin-versions.mjs` exits 0 with artifact-tools at 1.12.0 and social-media-tools at 2.22.1, and both plugins carry a CHANGELOG.md entry for their new version
- [ ] teach-framing.md names the extension seam where a future source would attach, and the skill still accepts exactly two sources
