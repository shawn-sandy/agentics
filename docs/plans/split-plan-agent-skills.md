...
- [x] `.claude-plugin/marketplace.json` sets `plan-agent` to `5.1.0`, and `python3 -c "import json;print('version' in json.load(open('kit/plugins/plan-agent/.claude-plugin/plugin.json')))"` prints `False`
- [x] `BASE_REF=main node scripts/check-plugin-versions.mjs` exits 0
- [x] `kit/plugins/plan-agent/CHANGELOG.md` has a `## 5.1.0` entry naming all five split skills
