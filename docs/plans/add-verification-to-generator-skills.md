...
1. Add a render assertion to the four `artifact-tools` skills by appending a step after each publish that fetches the returned artifact URL with `WebFetch` and asserts the page contains an expected marker (the plan title, the diff's first filename, the session date), reporting the failure with the URL on mismatch rather than reporting success. Why: these publish to an outward-facing URL and are the only skills here whose output the user cannot see locally, and `WebFetch` can already read `claude.ai/code/artifact/{uuid}` URLs via the user's login, so the check needs no new dependency. Verify: publishing a known-good plan through `plan-artifact` reports the fetched marker, and publishing HTML whose body is a single empty `div` makes the skill report failure instead of success.
