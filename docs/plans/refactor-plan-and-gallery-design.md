...
- [x] Every text token in the stylesheet measures at least 4.5:1 against its background in both the light and dark palettes
- [x] `--subtle: #9ca3af` no longer appears in `scripts/lib/plan-shell.mjs`
- [x] The literal string `html { scroll-behavior: auto; }` still appears in the rendered stylesheet
- [x] The rendered stylesheet contains no literal `{lowercase-word}` sequence that the unfilled-token check at test line 630 would flag
- [x] A first visit with no stored preference follows `prefers-color-scheme`, and a toggled choice survives a reload
- [x] The theme toggle button is hidden by the print stylesheet
- [x] The At-a-glance block renders inside `<div id="objective">` and `extractSections` returns an objective containing none of its text
- [x] Step `Verify:` text renders visible, outside any `<details>`, and still inside `<div class="verify-body">`
- [x] Each step card carries `id="step-N"` and keeps `class="step-card completed"` as a literal substring when done
- [x] Every `a.rail-step` href resolves to a step card in the same document, and each carries a visually-hidden state span
- [x] At 375px wide the step list is reachable through a disclosure rather than removed from the page
- [x] Section anchors in the sidebar still match `/<a href="#([a-z-]+)">/` with no other attribute
- [x] The two nav id arrays at `tests/plugins/test-build-plan-html.mjs:636` and `:639` pass unedited, and `id="progress-bar"` and `id="progress-label"` still exist exactly once each
- [x] Scrolling past the last section leaves no `.active` class set
- [x] `.steps-list::before`, `.step-verify-toggle`, and the `.plan-glance` negative-margin rule are absent from the stylesheet
- [x] No retired token name — including `--subtle`, `--grey-bg`, and `--border-mid` — remains in the stylesheet after step 6, and no selector references one
- [x] `diff` reports no differences between `scripts/build-plan-html.mjs` and `kit/plugins/plan-agent/scripts/build-plan-html.mjs`, and the same for `lib/plan-shell.mjs`
- [x] The three `build-index.sh` copies have identical checksums
- [x] Every `<a class="gallery-card"` in the regenerated plans index carries `data-month`, and in-progress cards precede the rest
- [x] `<a class="gallery-card"` remains the leading attribute pair on every card, and the `<p>N items</p>` and `<span>N items</span>` strings are unchanged
- [x] Filtering the gallery to a single status leaves no separator with zero cards under it
- [x] `node tests/plugins/test-plan-redesign.mjs` exits 0
- [x] `node tests/plugins/test-build-plan-html.mjs`, `node tests/plugins/test-extract-plan-spec.mjs`, `node tests/plugins/test-index-card-count.mjs`, and `bash tests/plugins/test-merge-gallery-index.sh` all exit 0
- [x] `BASE_REF=main node scripts/check-plugin-versions.mjs` exits 0 with plan-agent at 7.5.0
