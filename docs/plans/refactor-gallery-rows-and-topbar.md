...
- [ ] `docs/plans/index.html` renders one row per plan, not a card grid, and contains no `repeat(auto-fill` declaration
- [ ] `view-btn`, `list-view`, and the view-switching JavaScript are absent from every gallery template
- [ ] Every gallery card is a bare `<a class="gallery-card">` with `class` as its first attribute, no nested `<a>`, and no `<li>` wrapper
- [ ] `CARD_RE` from `scripts/merge-plans-index.mjs` matches every card in all four regenerated indexes, and the driver source is unedited
- [ ] `COUNT_RE` from `scripts/merge-plans-index.mjs` matches exactly twice in every regenerated index, and both captured numbers equal the card count
- [ ] Each in-progress plan card carries `data-steps-done` and `data-steps-total` matching its plan HTML's `step-card` counts, and renders a bar whose lit segments equal `data-steps-done`
- [ ] The `N / M steps` text is present with JavaScript disabled
- [ ] Every plan row announces its status as text to an accessibility-tree dump, and the glyph itself is `aria-hidden`
- [ ] No phase label is rendered anywhere — phases do not exist yet
- [ ] All four galleries render the sticky topbar with five tabs, each count matching its directory's file count, and exactly one `aria-current="page"`
- [ ] The topbar declares an opaque `background` before its `color-mix` value and stays legible with `color-mix` unsupported
- [ ] `--subtle`, `--grey-bg`, and `--border-mid` appear in no gallery template
- [ ] Every text token in all four gallery stylesheets measures at least 4.5:1 against its background in both light and dark palettes
- [ ] The media gallery renders in the light theme and honours a `plan-theme` preference stored by the plans gallery
- [ ] The artifacts gallery hides the status segmented control and renders no empty glyph column
- [ ] At 375px wide every gallery collapses to the prototype's stacked layout with no horizontal scroll
- [ ] The three `build-index.sh` copies have identical checksums, enforced by a test
- [ ] The `plans-library` SKILL.md card heredoc emits the same attributes and child spans as the build scripts
- [ ] `node tests/plugins/test-gallery-row-layout.mjs` and `node tests/plugins/test-build-index-parity.mjs` exit 0
- [ ] `node tests/plugins/test-index-card-count.mjs`, `bash tests/plugins/test-merge-gallery-index.sh`, `node tests/plugins/test-plan-redesign.mjs`, `node tests/plugins/test-build-plan-html.mjs`, and `node tests/plugins/test-extract-plan-spec.mjs` all exit 0 unedited
- [ ] `BASE_REF=main node scripts/check-plugin-versions.mjs` exits 0 with plan-agent at 7.6.0 and social-media-tools at 2.21.0
