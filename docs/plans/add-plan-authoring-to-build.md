...
- [ ] Choosing `Run as workflow` at that same Step 8 also stops the outer chain, with no in-session build started alongside the emitted workflow prompt
- [ ] A bare `/plan-agent:build` that reaches the chain via `None of these — author a new plan` asks for an objective before the proposal-versus-direct gate
