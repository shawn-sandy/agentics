...
Insert a new **Step 5b Interview** step between Step 5 Align and Step 6 Commit. This step analyzes the plan content for complexity, then runs 1–3 structured interview rounds using `AskUserQuestion` with dynamically generated questions. The step is a **standard part of the workflow** — it always runs unless explicitly opted out with `--no-interview` or `--quick`.

There is no `--interview` flag. The interview is not opt-in — it is a default stage of plan creation. The only flag is `--no-interview` (and `--quick` which implies it) to skip the step when speed matters. The old `--interview` flag (which delegated to the external `plan-interview` plugin after Step 8) is removed entirely. The standalone `plan-interview` plugin remains available for deeper post-hoc reviews.
...
- `--quick` bullet (line 16): expand to `--no-clarify --no-align --no-interview`; mention Step 5b
- `--interview` bullet (line 23): delete entirely and replace with `--no-interview` — skip Step 5b Interview. The interview is standard, not opt-in
- Delete the post-Step 8 delegation paragraph (line 56) — no more external delegation
...
- **Post-interview**: brief summary of findings, then `AskUserQuestion` to offer updating the plan HTML before proceeding to Step 6
...
- Added: Step 5b Interview step as a standard workflow stage (always runs by default)
...
- Lines 86–96: insert Step 5b in the workflow list between Align and Commit
...
- **No external dependency**: the plan content is already in-context from Step 2 Create. Delegating to plan-interview would re-read/re-analyze the file and run its full 6-step workflow unnecessarily
...
1. Run `/plan-agent:planning add a user profile page` (no flags) — confirm Step 5b runs, detects UI signals, runs Rounds 1 + 2a + 2b
2. Run `/plan-agent:planning --no-interview fix a typo` — confirm Step 5b is skipped
