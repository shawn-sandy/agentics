...
**The marker goes in the PR body, not the commit body.** An earlier draft of
this plan required both, which is not reachable from the files it changes:
`ship-autonomous` delegates commits to `commit-agent`, whose Step 4 is a
single-`-m` `git commit -m "<message>"` against a message capped at 72
characters — there is no commit body to write into. Giving `commit-agent` one
is a behaviour change to a skill with several callers and belongs to its own
plan. The PR body is also where the claim actually misleads someone: a
reviewer reads it, a commit trailer they do not. So the requirement is the
session output plus one line in `pr-agent`'s Test Plan template, and
`pr-agent` joins the file list to make that reachable.
