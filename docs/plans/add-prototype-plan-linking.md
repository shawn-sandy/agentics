...
- kit/plugins/plan-agent/scripts/lib/plan-shell.mjs (modified) — optional `plan-prototype` meta tag and the header link
- kit/plugins/plan-agent/scripts/build-plan-html.mjs (modified) — thread the `prototype` frontmatter key into both `metaTags()` and `header()`
...
4. Add a write-back step to `skills/prototype/SKILL.md` that runs **before** the prototype HTML is written in Step 6, resolving the plan-path input's `.html` to its sibling `.md` spec by extension swap and writing `prototype: docs/prototypes/<slug>.html` plus `proto-model: <compact single-line JSON>` into that spec's frontmatter — plan path only, skipped for idea, image, and Figma inputs. The JSON must be single-line: never pretty-printed, never containing a raw newline or a bare `---`. Why: writing the prototype fires `dispatch.py`, so a write-back placed after it would run the drift check against a plan that has no `proto-model` yet and warn on every single generation; and the frontmatter parser is a naive line scanner, so an embedded newline or `---` silently truncates the block and corrupts `status` and `created` for all three consumers that re-scan it. Verify: run the skill against a fixture plan spec, confirm both keys land on one line each, that `node scripts/extract-plan-spec.mjs` still parses the plan, and that no drift warning is printed during generation.
5. Thread `prototype` through the renderer in three places: read `parsed.metadata.prototype` in `build-plan-html.mjs` near the existing `parsed.metadata.created` handling; pass it to `shell.metaTags()`, which emits `<meta name="plan-prototype">` conditionally; and extend `shell.header()`'s parameter list **and its call site in `build-plan-html.mjs`** to render an anchor inside `.plan-header-actions` beside the effort badge, with visible text `View prototype` and `aria-label="View the interactive prototype for this plan"`. Compute its `href` as `../prototypes/<slug>.html`, converting from the repo-relative frontmatter value, since plans and prototypes are sibling directories under the Pages root. Why: `metaTags()` and `moreWaysDrawer()` are the existing precedents for conditional emission — `header()` has a fixed unconditional parameter list today, so the optional slot there is new territory and easy to miss; and the Verification section requires the link actually resolve when clicked, which a repo-relative href would not. Verify: render a spec carrying `prototype:` and confirm the meta tag, a header anchor with non-empty accessible text, and a working relative href all appear; render one without it and confirm none do.
6. Add a prototype chip to the plans gallery card in `build-index.sh` alongside the existing `card-date` span, as a text-bearing span matching the existing `status-chip` / `type-chip` / `effort-chip` pattern, carrying a `title` explaining that the prototype opens from inside the plan. It must **not** be an anchor — the whole card is already wrapped in `<a class="gallery-card">` at line 149, and a nested `<a>` is invalid HTML that browsers silently unnest. Why: an icon-only chip gives screen reader and voice-control users no signal a prototype exists, while a nested anchor breaks the card's click target; a plain text node inside the card anchor is read for free as part of its accessible name. Verify: the emitted card contains a chip with non-empty text and no `<a` nested inside another `<a`.
...
- Unit: renderer back-compat. File: tests/plugins/test-build-plan-html.mjs; Targets: metaTags, header; Key cases: a spec with no `prototype:` key renders byte-identical to its pre-change output

## Acceptance Criteria

- [ ] A plan spec carrying `prototype:` renders a `plan-prototype` meta tag and a visible header link whose href resolves to the prototype file from the plan's own directory.
