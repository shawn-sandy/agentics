...
- [x] A plan spec carrying `prototype:` renders a `plan-prototype` meta tag and a visible header link whose href resolves to the prototype file from the plan's own directory, including when the plan lives in a custom `plansDirectory`.
- [x] The two renderer files and the three plans-index builders remain byte-identical across all their copies after the change.
- [x] Running `/plan-agent:prototype` against a legacy HTML plan with no sibling `.md` still writes the prototype, creates no spec file, and prints a notice explaining how to get the back-link.
- [x] A plan spec without `prototype:` renders byte-identically to its output before this change.
- [x] The header prototype link and the gallery prototype chip both expose non-empty accessible text.
- [x] Every prototype generated from the skeleton contains a parseable `#proto-model` JSON block, and its `proto-source` holds the repo-relative path of the plan's markdown spec.
- [x] Running `/plan-agent:prototype` against a plan writes single-line `prototype:` and `proto-model:` keys into that plan's `.md` frontmatter before the prototype HTML is written, and prints no drift warning during generation; running it against an idea, image, or Figma input writes to no plan.
- [x] The plans gallery card shows a prototype chip, and the emitted card HTML contains no nested anchor.
- [x] `check-prototype-drift.py` reports divergence between a prototype's model and its rendered columns, and between a prototype's model and its plan's copy, naming both files and the diverging field.
- [x] The drift hook exits 0 on every input, including missing files, missing blocks, out-of-tree paths, and malformed JSON.
- [x] `BASE_REF=main node scripts/check-plugin-versions.mjs` exits 0 with plan-agent at 4.4.0.
