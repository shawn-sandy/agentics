...
12. [x] Run `bash tests/run-all.sh` Why: the two new tests must pass alongside the existing 77 without regressing any of them Verify: the runner's final line reports 0 failed and the two new test paths appear in the PASS list.
13. [x] Run `bash scripts/verify.sh` in agentics and read its full output Why: a gate that passes its own unit tests but does not actually run here is not a gate Verify: it exits 0, prints `SKIP (not configured)` for typecheck, lint, and e2e, and prints real results for the unit stage and the three marketplace stages.
14. [x] Run `git fetch origin && BASE_REF=main node scripts/check-plugin-versions.mjs` Why: the script reads `origin/main` directly, so a stale remote-tracking ref compares against the wrong base and passes a bump that CI will reject Verify: it exits 0 and names `code-testing-agent 3.5.2 -> 3.6.0`.
15. [x] Mutation-check the gate itself by breaking the failure propagation in `scripts/verify.sh` so a failing stage returns 0, confirming `tests/test-verify-gate.sh` goes red, then restoring from the scratchpad copy under a `trap` that also restores on interrupt Why: the plan builds a mutation check, so its own objective test has to survive one — and this is the dogfood that proves the protocol in step 7 works Verify: the mutated run of `bash tests/test-verify-gate.sh` exits non-zero naming the fail-fast assertion, and after restore `cmp -s "$SCRATCH/verify.sh" scripts/verify.sh` exits 0 and the test passes again. Do not substitute `git diff --quiet scripts/verify.sh` here: the file is untracked until this branch is committed, so that check would pass on an unrestored mutation and the dogfood would prove nothing.
...
- [x] `bash scripts/verify.sh` exits 0 in agentics and prints an explicit result or SKIP line for every stage
- [x] The same script, unmodified, prints four SKIP lines and exits 0 in a project with no toolchain
- [x] A failing stage makes the gate exit non-zero and stops it from running later stages
- [x] `bash tests/run-all.sh` reports 0 failed with both new tests in the PASS list
- [x] `.claude/skills/verified-change/SKILL.md` is byte-identical to the plugin copy and a test fails if they diverge
- [x] `scripts/verify.sh` is byte-identical to the plugin's `assets/verify.sh` and a test fails if they diverge
- [x] `BASE_REF=main node scripts/check-plugin-versions.mjs` exits 0 with code-testing-agent at 3.6.0
- [x] `node scripts/build-readme-table.mjs --check` exits 0
- [x] `CLAUDE.md` states that merging is not proposed until the gate is green and VERIFICATION is filled in
- [x] The skill's own objective test survives a deliberate mutation of the gate's failure propagation
- [x] The gate detects every stage relative to the directory it is invoked from, so running it inside `tests/fixtures/verify-gate-bare/` terminates instead of re-entering `tests/run-all.sh`
- [x] `find tests/fixtures/verify-gate-bare tests/fixtures/verify-gate-failing \( -name 'test-*.sh' -o -name 'test-*.mjs' -o -name '*.test.mjs' \)` prints nothing, so `tests/run-all.sh` never picks the failing fixture up as a suite member
- [x] The mutation-restore proof is `cmp -s` against the scratchpad copy and fails loudly on an unrestored file even when that file is untracked
- [x] `kit/plugins/code-testing-agent/bin/install-verify-gate` copies the gate into a scratch repo without any `${...}` expansion in the documented command
- [x] `diff -r kit/plugins/code-testing-agent/skills/verified-change .claude/skills/verified-change` prints nothing, references included
...
- shellcheck detection — gated on a `.shellcheckrc`, not on the binary being installed, so the gate does not turn red for everyone the day someone runs `brew install shellcheck`; this repo has no `.shellcheckrc`, so lint reports SKIP.
- `bin/install-verify-gate` shape — it is a pure `exec` wrapper over `assets/install-verify-gate.sh`, because check 3 of `tests/plugins/test-no-shell-expansion.sh` parses every wrapper for an `exec <interp> "$(dirname "$0")/…"` line and fails a wrapper that carries logic of its own.
- Re-entry marker escape — `tests/test-verify-gate.sh` invokes the gate through `env -u VERIFY_GATE_ACTIVE`, because that test itself runs under the gate whenever the unit stage is `tests/run-all.sh`; without the escape the marker would refuse the very invocation the test exists to make. It is the one site allowed to clear it, and check 2 asserts the marker still refuses everywhere else.
- Project-local copy scope — `.claude/skills/verified-change/` is a whole-directory copy including `assets/`, since the acceptance criterion requires `diff -r` against the plugin directory to print nothing; that makes three copies of the gate, all held in lockstep by the parity test.
- Version-guard verification method — `node scripts/check-plugin-versions.mjs` compares `origin/main...HEAD`, which is empty on an uncommitted branch and therefore passes vacuously; the 3.5.2 to 3.6.0 bump was verified by calling the script's exported `findViolations` against the real working-tree change set. The CLI form becomes meaningful once the work is committed.
- Silent-failure bug found by the dogfood — proving the parity test fails on a forked gate showed it exiting non-zero with no message, because `diff | head | sed` under `set -e` killed the script before the report. Fixed with `|| true`; the proof is what caught it.
