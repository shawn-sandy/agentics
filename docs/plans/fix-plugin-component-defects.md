...
- `kit/plugins/plan-agent/hooks/build-index.sh` (modified) — resolve templates via CLAUDE_PLUGIN_ROOT
...
7. Replace `find_templates_dir()` in `kit/plugins/plan-agent/hooks/build-index.sh:36` with a direct resolution against `$CLAUDE_PLUGIN_ROOT/templates`. Why: it currently walks all of `~/.claude/plugins` and the project root on every plan write, which is unbounded work whose cost scales with how many plugins the user has installed and how large their repo is, and `$CLAUDE_PLUGIN_ROOT` is the supported way to find plugin-relative paths. Verify: the index still rebuilds correctly on a plan write, and the hook performs no directory walk (confirm by inspection and by timing a plan write in a repo with many installed plugins).
...
- [ ] `build-index.sh` resolves templates without walking the filesystem
