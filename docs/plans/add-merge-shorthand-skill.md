...
- Unit: hook regex boundaries. File: tests/plugins/test-merge-shorthand.sh; Targets: hooks/merge-shorthand.py; Key cases: exact `merge?`, surrounding whitespace, uppercase `MERGE?`; negative cases `merge`, `please merge?`, `merge? now`, `can you merge this?`, and prose containing "merge?" mid-sentence

## Acceptance Criteria

- [x] `kit/plugins/git-agent/skills/merge/SKILL.md` exists with valid frontmatter, a ≤200-char three-part description, and logic that merges only when MERGEABLE with passing required checks — never with `--delete-branch`
- [x] The skill runs the project's lint script (non-`fix`, non-`watch`) before merging, skips with a note when none exists, and on lint failure stops and asks — never auto-`--fix`es
- [x] The skill asks for explicit merge approval even when everything is green, and merges with `--match-head-commit <headRefOid>`
- [x] `echo '{"prompt":"merge?"}' | python3 kit/plugins/git-agent/hooks/merge-shorthand.py` emits routing context; a prompt merely containing "merge" emits nothing
- [x] `kit/plugins/git-agent/hooks.json` is valid JSON wiring the script via `${CLAUDE_PLUGIN_ROOT}` under `UserPromptSubmit`
- [x] `bash tests/plugins/test-merge-shorthand.sh` exits 0
- [x] `marketplace.json` shows git-agent `4.4.0` and `BASE_REF=main node scripts/check-plugin-versions.mjs` passes
- [x] CHANGELOG.md, README.md, and the CLAUDE.md plugin table document the new skill and shorthand
- [x] The `feedback_merge_behavior` memory note is reduced to a pointer at `git-agent:merge`
