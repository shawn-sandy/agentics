...
The republish keys are the sharp edge. Each command writes to a *different*
key on the same shared session record: `product-doc` and the older skills use
`artifact-url:`, `eng-recap` uses `eng-artifact-url:`, and `team-recap` has its
own. Collapsing the commands must not collapse the keys — two commands writing
the same key would silently overwrite each other's published artifact. Step 3
pins each key explicitly and the objective test asserts all three remain
distinct.
...
- kit/plugins/artifact-tools/commands/product-doc.md (modified) — reduce to product framing + `artifact-url:`
...
4. Confirm the three republish keys remain distinct and match what the shared session record already uses. Why: two commands sharing a key would overwrite each other's published artifact with no error. Verify: `grep -o '[a-z-]*artifact-url:' kit/plugins/artifact-tools/commands/*.md | sort -u` returns three distinct key names.
...
6. Write `tests/plugins/test-recap-command-dedupe.sh` asserting the three commands share fewer than 50 identical lines, each is under 500 words, each names a distinct republish key, and `references/recap-core.md` exists. Why: without a check, the next feature added to all three commands re-introduces the duplication. Verify: `bash tests/plugins/test-recap-command-dedupe.sh` exits 0; pasting 60 lines of recap-core back into two commands makes it exit 1.
...
- [ ] The three republish frontmatter keys remain distinct from one another
