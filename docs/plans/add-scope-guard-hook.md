...
- [x] `npm run fix:all` is blocked when its script resolves to a repo-wide formatter, and the block message names the scoped alternative.
- [x] The same block applies to all eight runner spellings — `npm`/`pnpm`/`yarn`/`bun`, each with and without an explicit `run` token.
- [x] A formatter invoked with an explicit path is never blocked.
- [x] A bare `git stash pop` or `git stash apply` is blocked, and the message names `git stash list`; an explicit `stash@{N}` passes.
- [x] No rule blocks `rm`, `curl`, `git reset`, or `git checkout` — the guard's scope is exactly the two documented patterns.
- [x] A non-Bash payload, and a Bash command carrying no trigger token, exit 0 without reading any file.
- [x] A blocked pattern appearing inside a `git commit -m` message, an `echo`, or a `grep` argument is not blocked — only an actual invocation is.
- [x] `.claude/no-scope-guard` at the repo root disables every rule.
- [x] Blocks exit 2 with the rule and the safe alternative on stderr.
- [x] `lint-before-commit.py` still fires on `git commit` with its existing timeout unchanged.
- [x] The README documents both rules, both escape hatches, the desktop-app caveat, and a `.claude/lint-gate.json` test-command example.
- [x] `bash tests/plugins/test-scope-guard.sh` reports zero failures.
