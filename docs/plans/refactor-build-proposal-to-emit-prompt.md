...
7. Rewrite `build-proposal`'s Step 6 to dual-write — invoke `write-prompt` with the proposal content and the Step 5 bypass token, derive the saved prompt path deterministically as `{prompts-dir}/proposal-{slug}.md` — **date-free, slug is the identity** — rather than reading anything back, and additionally write the legacy `docs/proposals/<slug>.md` carrying a deprecation banner naming the prompt as authoritative — then rewrite Step 8 to hand off the prompt path, and retarget `--dir` to the prompts directory. Why: `Skill()` has no documented return value, so deriving the path removes the transcript dependency entirely; and the date **must** be omitted for this type because a living document spanning two calendar days would otherwise derive a different filename on round two and silently write a second file instead of rewriting the first — the `created:` and `modified:` frontmatter keys carry the dates instead. Verify: a run started on one date and continued on a later date resolves to the same single file, and `ls {prompts-dir}/proposal-*` shows exactly one file per slug.
...
- [ ] `build-proposal` derives the prompt path from the slug alone without reading `write-prompt`'s output back, and a loop spanning two calendar days resolves to one file rather than two.
