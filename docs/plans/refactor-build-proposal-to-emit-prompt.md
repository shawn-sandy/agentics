...
- [ ] `write-prompt` honours a caller-supplied `--out <path>`, writing to exactly that path and bypassing its own directory resolution and intent-slug derivation.
- [ ] A hand-edited prompt file triggers a confirmation prompt before the next round overwrites it, detected via `generated-sha:` rather than a git-committed baseline, so an uncommitted previous round does not false-positive.
...
- [ ] `build-proposal` passes an explicit output path to `write-prompt` rather than reading its output back or independently deriving one, and the path it reports is byte-identical to the file actually written.
