...
> convention. Rename it to **`create-plan-mode-plugin.md`** with `git mv`
> before the implementation commit. It is left as-is here only so the harness
> can bind `ExitPlanMode` to this path.
...
4. **Port `hooks/validate-plan-filename.py`.** Copy the script from
   ...
   - *Verify:* `echo '{"tool_input":{"file_path":"docs/plans/foo-bar.md"}}' | python3 kit/plugins/plan-mode/hooks/validate-plan-filename.py; echo "exit=$?"` → exit 2 (bad name); same with `add-dark-mode-toggle.md` → exit 0.
...
7. **Register in `marketplace.json`.** Add a `plan-mode` entry to the `plugins`
   ...
   - *Verify:* `python3 -c "import json;d=json.load(open('.claude-plugin/marketplace.json'));print(d['version'], len(d['plugins']), [p['name'] for p in d['plugins'] if p['name']=='plan-mode'])"` prints `3.8.0 17 ['plan-mode']`. (The project's settings.json auto-validates this JSON on Write.)
...
- [ ] `kit/plugins/plan-mode/` exists with `.claude-plugin/plugin.json` (has
...
- [ ] The `authoring-plans` skill carries the complete Plan Mode workflow
      (§0–§7), Required Structure, and Writing Style, and references its bundled
      `SKELETON.md`.
- [ ] The hook rejects non-`verb-target` plan filenames (exit 2) and passes valid
      ones (exit 0), skipping `status: completed` plans, with `plansDirectory`
      resolved from the project's `.claude/settings.json` first.
- [ ] `marketplace.json` lists `plan-mode` at `0.1.0`, total 17 plugins, metadata
      version `3.8.0`, and remains valid JSON.
- [ ] Root `CLAUDE.md` reflects 17 plugins / v3.8.0 with a `plan-mode` table row.
- [ ] No existing plugin (especially `plan-interview`) is modified.
...
2. **Hook behavior (end-to-end):**
   - `echo '{"tool_input":{"file_path":"docs/plans/wobbly-bachman.md"}}' | python3 kit/plugins/plan-mode/hooks/validate-plan-filename.py` → exit 2 with a rename message.
   - `echo '{"tool_input":{"file_path":"docs/plans/create-plan-mode-plugin.md"}}' | python3 …` → exit 0, silent.
   - A file under `docs/plans/` with `status: completed` frontmatter and a bad name → exit 0 (skip).
...
- Rename this plan file to satisfy §4 before commit:
  ```text
  In the agentics repo, git mv docs/plans/revire-this-plan-mode-reflective-thacker.md
  docs/plans/create-plan-mode-plugin.md (and migrate any matching .html artifact),
  then stage the rename. This enforces the plan-mode verb-target filename rule on
  the plan that introduces the plan-mode plugin.
  ```
...
- Initial version number:
  ```text
  Confirm whether the new plan-mode plugin should debut at 0.1.0 (pre-1.0, matching
  code-share's 0.x line) or 1.0.0. Recommend one based on how the agentics-kit
  marketplace versions brand-new single-skill plugins, and apply it consistently in
