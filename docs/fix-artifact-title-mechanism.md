...
[artifact-tools 1.1.0](../../kit/plugins/artifact-tools/CHANGELOG.md) added `references/titles.md` as the single source of artifact title rules, and told authors to set the title "as the page's `<title>` for HTML sources, or as frontmatter `title:` for Markdown sources." The second half is false. The Artifact renderer does not parse YAML frontmatter. Publishing a Markdown
