...
- **What happens** — Resolves the repo path from the argument, `~/.claude/settings-sync.json`, or a prompt, then scans `settings.json`, `CLAUDE.md`, `keybindings.json`, `rules/`, `commands/`, `skills/`, `hooks/`, `agents/`, `output-styles/`, `scripts/`, and `reference/` for secrets before copying them in. Any ignore rule missing from `.gitignore` is appended and untracks whatever it already covers. Stages the copy and commits only when `git diff --cached --quiet` finds a real change, writing `.settings-sync-meta.json` and pushing when a remote exists. Any repo-root entry that is neither a backup target nor a control file — a `plans/` directory left by an older version, say — is listed in the report under `Not a backup target (left in repo)`; nothing is deleted, since a hand-added entry is deliberate.
