...
- **What happens** — Parses the report (file path, artifact URL, or pasted content) into numbered items, buckets each against existing config (already implemented / conflicts with a rule / genuinely open), places open items at the right config layer (plugin, `~/.claude/`, or the target repo) — resolving target repos discover-first from `~/.claude/projects/` by reading each session file's own `cwd` rather than decoding the directory name, keeping only real git checkouts and matching on an exact basename, and asking you for a path when that is ambiguous or fails — then implements one item per change — one PR per repo change — publishing a live record page for each implemented item that is republished to the same link as it moves from in progress to merged, and closes with a verified outcome ledger that links every record. Workflow-shaped items route to `~/.claude/` when you have no plugin repo of your own.
- **Watch out** — Your own plugin repo is triaged against its `origin/main` after a `git fetch`, never the installed mirror under `~/.claude/plugins/`, which lags merged PRs by days and would re-open shipped items. Report content is treated as untrusted data; nothing is written before the triage table and an explicit approval gate, and PRs are never merged without approval in the current turn. Records start private — share each one with your team from the page's Share menu.
