...
- **Command** — `/plan-agent:build [<plan.md|plan.html>] [<objective>] [--type feature|fix|refactor|docs|chore] [--dir <path>] [--sequential] [--workflow] [--max <n>] [--worker-model <alias>]`
- **Say it instead** — "implement the plan in docs/plans/add-dark-mode.md"
- **What happens** — Resolves the plan (with no path it first chains into authoring one), sets `status: in-progress`, then branches on the plan's shape. A spec with two or more `### Lane:` headings commits the spec and dispatches one background worktree worker per lane (`--max` at once, default 3, cap 5, on `--worker-model`, default `sonnet`), verifies each LANE REPORT against `git log`, audits the lane branch's diff against its `owns:`, merges `--no-ff` in `after:` order, runs the `lead` lane in your session, and then runs the acceptance-criteria, end-to-end verification, and completion checklist gates once. Anything else walks each step sequentially, ticking the markdown spec and re-rendering the HTML, then runs the same three gates. `--workflow` (or `workflow: always`, or 6+ lanes) swaps the Agent dispatcher for the Workflow-engine script.
- **Watch out** — The markdown spec is the source of truth: ticking a box in the browser is discarded on the next re-render. A sequential run leaves everything uncommitted unless you ask it to commit; a laned run leaves the tree committed on the plan branch, unpushed, and stops naming the file and lane if a worker touched a path outside its `owns:`. `--sequential` or `workflow: never` keeps a laned plan on the sequential path. For a plan published to claude.ai the completion gate renders to the scratchpad and checks that, never writing the `<stem>.html` the author chose not to publish — a sibling would flip the plan's gallery card off the shared artifact onto a local path.
