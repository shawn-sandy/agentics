...
- **Watch out** — The push only happens on an explicit approval prompt; a plain pre-commit hook failure is reported verbatim and stops the run — no retry, no `--no-verify`, no push reconciliation via pull, rebase, or force. A block from git-agent's own lint gate is different: invoked directly, it asks whether to fix and retry (fixing only the files this commit touches, re-staging just those, up to two rounds); invoked by another skill, it reports the block and stops for the caller to fix. Either way it never creates `.claude/no-lint-gate` or edits `.claude/lint-gate.json` to get past the gate.
...
- **Say it instead** — Not available; this skill is command-only (`disable-model-invocation: true`).
- **What happens** — Guards against detached HEAD, the default branch, and an unauthenticated `gh`; stops on an existing open PR, pushes (with `-u` when there is no upstream), runs `git-agent-extract-plan-issues` to add `Closes` lines for any plan on the branch that is marked `completed` — reading a `.md` spec's `issue:` frontmatter as well as a rendered plan's `plan-issue` meta tag, so an artifact-delivered plan with no `.html` still closes its ticket — runs an adversarial pre-PR review of `git diff <base>...HEAD` in a fresh-context subagent (eleven checks: no-op edits, vacuous test assertions, self-introduced regressions, unsafe auth lookups, secrets, accessibility regressions, sort tie-breakers, unvalidated numeric input, stale derived state, timezone-dependent date anchors, and scripts that continue after a failed step), pushes any review fix commit, then prints the URL from `gh pr create`.
...
- **Watch out** — Any BLOCKED row stops the run before any mutation and is never auto-remediated (no re-auth, no stash, no `.env` copy); a plain pre-commit hook failure is reported verbatim and stops. A block from the lint gate is different: `ship` asks whether to fix and retry, fixing only the files the commit touches, for up to two rounds, and never disables the gate. `--no-review` skips the self-review; when it runs, only a confirmed secret blocks the ship — everything unconfirmed is reported, not fixed. A reviewer subagent that returns no report (partial, empty, or missing its `### Summary` heading) is not re-dispatched — the checklist runs inline and the step report says so.
...
- **Watch out** — The context guard is skipped on a short session, and picking `clear` **stops** the run — a skill cannot clear its own context, so you run `/clear` and re-invoke; `background` chains `ship-bg` then `ship-ci-bg` for fresh context windows but still returns to the foreground to merge. Failing tests or lint, or any console or server error, stop the pipeline; a plain pre-commit hook failure does too. A lint-gate block at any delegated `commit-agent` commit is handled instead: fix only the files that commit touches with a direct edit (never a `--fix` script — nothing may be committed unreviewed), retry, and give up after two rounds. Autofix elsewhere is capped at 3 attempts per failing check and limited to lint, typecheck, and peer-deps; the merge needs an explicit approval on green pinned to `headRefOid`, and branch deletion requires its own separate yes.
