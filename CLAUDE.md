...
| `plan-agent` | Skills + Hooks | Explicit `/plan-agent:implementation-plan <objective>` skill (manual-invoke, `disable-model-invocation`) runs Steps 1–8 plan workflow with built-in structured interview (Step 5b); automatic `verb-target` filename hook on `Write`/`Edit` |
