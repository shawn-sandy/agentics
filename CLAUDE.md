...
| `git-agent` | Skills + Agents + Hooks + Commands | Branch creation, commit, PR, and ship workflows; `merge` skill checks PR readiness (MERGEABLE, green checks, lint gate) and merges only with explicit approval, never `--delete-branch` — typing `merge?` routes to it deterministically via a `UserPromptSubmit` hook; background commands: `commit-bg`, `pr-bg`, `ship-bg`; `ship-autonomous` for supervised full pipeline; `create-issue` skill files GitHub/GitLab issues from any context (selection, session, bug, feature, plan — a plan file becomes a checklist-style ticket) with host auto-detection and confirmation gate |
...
| `plan-agent` | Skills + Agents + Hooks + Commands | `/plan-agent:implementation-plan <objective>` runs Steps 0–8 plan workflow with built-in interview; `/plan-agent:build [<plan>]` implements an existing plan end-to-end — steps, spec ticks, re-render, and the acceptance/end-to-end/completion gates (Step 8's `Implement now` delegates here; implementation-plan never writes source files); `/plan-agent:review-plan` spawns a seven-reviewer Agent Team; `/plan-agent:review-plan-bg <path>` runs the review team in the background; `/plan-agent:finalize-plan` reviews and marks plans completed with per-criterion verification; `/plan-agent:build-proposal <idea>` turns a vague idea into a decision-complete `docs/proposals/<slug>.md` via an 8-step research→decide loop + Tier 0/1/2 right-sizing gate, then hands off to implementation-plan; `/plan-agent:write-prompt` generates AI prompts from Anthropic best practices (command-only); accepts issue URLs/`#n` to seed plans and `.md` plan paths for conversion to HTML; offers optional tracking-issue creation at the end of every plan via `git-agent:create-issue plan <spec>`; `plans-library` builds filterable gallery; `plans-open` reopens gallery; `/plan-agent:setup-sites` scaffolds the GitHub Pages deploy pipeline (workflow, `.nojekyll`, hub, preview script) into any repo so `docs/` HTML publishes to a public URL; `/plan-agent:prototype <plan.html\|idea>` generates a runnable, framework-free static-HTML prototype under `docs/prototypes/` (inline JSON seed + per-prototype localStorage, escaped output, a11y baked in) auto-indexed into a Prototypes gallery; absorbed from the former `plan-interview` plugin (v4.0.0): `/plan-agent:documenting-plans` generates developer docs from completed plans (with a `plan-documenter` batch agent), `/plan-agent:markdown-to-html` converts any markdown or plan to a rich standalone HTML document, `/plan-agent:plan-status` writes lifecycle status for a single plan or a whole directory in bulk (`--all`/`--force`), `/plan-agent:plan-maintenance` archives completed plans and regenerates the index, and `/plan-agent:deep-grill` walks each plan decision branch node-by-node; automatic filename hook + plans/prototypes gallery index auto-rebuild hooks + an ExitPlanMode nudge to stress-test after exiting plan mode |
