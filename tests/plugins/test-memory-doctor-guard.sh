...
skill_bundle "$ADVISOR" | grep -qF 'git --no-pager diff -- "$TARGET"' || MISSING="$MISSING advisor:diff"
...
extract_check() {
  skill_bundle "$1" \
    | awk '/^python3 - "\$TARGET" <<'"'"'EOF'"'"'$/ {grab=1; next} grab && /^EOF$/ {exit} grab {print}'
}
