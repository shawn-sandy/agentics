...
- **Local preview tooling** — `scripts/serve-docs.sh` plus the auto-port VS Code configs in `.claude/launch.json` (`plans-gallery`, `media-library`, `docs-all`), which use `autoPort: true` and bind a free port at start time. Documented in the repo `README.md` under "Browsing Docs Locally".
