...
- **8-step loop** — Frame → **Confirm the ask (Step 1b gate)** → Fan out research (parallel) → Synthesize the core finding → Separate facts from decisions → **Choose the approach (Step 4b gate)** → Resolve decisions (recommendation-first) → Author the artifact → Deepen on request → Converge & hand off. Step 0 self-bootstraps out of plan mode.
...
- **Step 4b chooses the approach before anything is authored** — once Step 4 has said the open items are decisions, the candidate solutions drafted at Step 3 (two to four, the baseline *keep the current approach* always among them) go to the human in one recommendation-first `AskUserQuestion`. The pick becomes the first Locked decision, naming what it was chosen over; the rejected candidates stay as columns of the Side-by-side table; Step 5 asks only the questions the chosen approach raises and Step 6 authors it alone. At Tier 1, which omits both slots, the pick closes the Context section as one line. An "Other" answer is a new candidate: restated, added to the table, locked.
