...
- Include `<meta name="plan-status">`, `<meta name="plan-type">`, `<meta name="plan-created">`, `<meta name="plan-repo">`, and `<meta name="plan-reinvoke">` tags for machine readability; include `<meta name="plan-issue" content="<url>">` when the plan was seeded from an issue reference (omit otherwise)
...
- Include a **re-invoke prompt row** (`<div class="plan-reinvoke">`) immediately below the `.objective-card` and before `.progress-wrap`. It must contain: the `{reinvoke-cmd}` value in `<code id="reinvoke-cmd" aria-label="Re-invoke command">`, and a Copy button with `onclick="copyCmd(this)"`. The copy button is hidden via CSS when `data-status="completed"`. The row is suppressed in `@media print`.
