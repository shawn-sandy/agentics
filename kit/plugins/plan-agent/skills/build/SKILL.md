...
`## Completion Report`. Never `checked` attributes in the HTML, never
browser-only persistence — the next re-render discards them.
...
Non-zero exit means the spec broke the format: fix the markdown, never the
HTML.

## Step 1 — Resolve the plan

Read `references/resolve-plan.md` now and follow its Step 1. A missing path
stops — never implement a different plan, never enter Step 1b. Headless, take
each gate's named default and log it. Already `status: completed` → ask.
...
artifacts uncommitted — **never clean either up.**
...
`references/design-spec.md` before writing code — its artboards are the target.
...
Report what was implemented, what ran, and the final status, then stop. A sequential run leaves everything uncommitted in the working tree; a
laned run leaves it committed on the plan branch, unpushed. Commit or push
only if the user asks, and never on `main`/`master`.
