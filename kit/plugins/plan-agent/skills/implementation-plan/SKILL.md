...
8. **Implement, Edit, or Exit** — After Step 7 completes, always ask the user what to do next.
   ...
   **If the user chooses `Background`:** Invoke `Skill(skill: "plan-agent:review-plan", args: "<plan path> --background")`, passing the current plan's relative path with the `--background` flag so the review team runs detached. Return to the Step 8 menu immediately and note that the user can reopen the plan to see changes once the detached run finishes. Leave plan `status` at `todo`.
