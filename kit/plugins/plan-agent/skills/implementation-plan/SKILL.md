...
```
   ...
   **Full implementation prompt (Copy behavior):** The Copy button on the implement prompt row does **not** copy the short `{implement-prompt}` text. Instead, it calls `buildImplementPrompt()` which builds a concise action-oriented prompt from the plan's live DOM state, including: (1) the short implement prompt as the opening line, (2) a status summary with step and criteria progress counts, and (3) numbered instructions directing the implementer to read the plan file directly (a self-contained HTML file — no repo-local script required), implement todo steps, verify and check off acceptance criteria, and complete the completion checklist — all directly in the plan file.

   **Goal prompt:** After computing the implement prompt, also compute a goal prompt — an outcome-driven sibling that frames the work as a *goal to achieve* rather than steps to execute, giving the implementer latitude to deviate from the steps when a better path to the same outcome exists. Unlike the workflow prompt, the goal prompt is **always generated** — no flag, no complexity heuristic — because every plan has an objective worth pursuing. The format is: `Achieve this goal: <objective>. The plan at <filepath> describes one approach — use it as reference, but optimize for the outcome`, where `<objective>` and `<filepath>` are the same condensed objective and relative path used in the implement prompt. Example: `Achieve this goal: Ship a dark-mode toggle that persists across all three themes. The plan at docs/plans/add-dark-mode-toggle.html describes one approach — use it as reference, but optimize for the outcome`. Store this as `{goal-prompt}` and fill the skeleton placeholder (HTML-escape it wherever it is inserted, per the escaping rule in HTML Output Requirements).

   **At a glance:** After the objective is settled, write a 2–3 sentence plain-language summary and fill the `{at-a-glance}` placeholder with it. It renders in the `.plan-glance` block, which the skeleton places as a *sibling* immediately after `div#objective` — never nested inside it (the plan-spec extractor (`scripts/lib/plan-spec.mjs`) derives the objective spec from `div#objective`'s inner HTML, so nesting would pollute extractor output; keep the sibling placement intact when filling). The objective and the glance have distinct jobs and must **never restate each other**: the objective is the one-line *what*; the at-a-glance explains *why it matters* and *how we'll know it worked*. If the glance repeats the objective sentence, rewrite it. Write it for a reader who wasn't in the planning session — no unexplained jargon (see Writing Style). HTML-escape it like every other filled placeholder.

   **Effort level:** After drafting all steps (and the file-tree), compute an effort level — an auto-derived estimate of how much work the plan represents, surfaced as a colour-coded badge, a header meta-row chip, and a `plan-effort` meta tag. It is **always generated** — no flag, no author input. Derive it deterministically from plan size: weigh the **step count**, the number of **distinct files** in the file-tree, and the **Step 5b interview complexity tier** (short / medium / complex), then map to one of three buckets:
   - **Low** — ≤3 steps **and** ≤2 distinct files (and the interview tier is not `complex`).
   - **High** — ≥7 steps, **or** ≥6 distinct files, **or** the interview tier is `complex`.
   - **Medium** — everything in between.

   When Step 5b was skipped (`--quick` / `--no-interview`), derive from the step and file counts alone. The result fills **two distinct placeholders** — keep them separate or the badge loses its colour:
   - `{effort}` — the **capitalised display label** (`Low` | `Medium` | `High`), used only in the visible badge and meta-row chip.
   - `{effort-value}` — the **lowercase machine value** (`low` | `medium` | `high`), used for the `data-effort` attribute on `<html>` and the `<meta name="plan-effort">` content. The CSS selectors are lowercase (`[data-effort="high"]`), so a capitalised value here would render an uncoloured badge.

   **Workflow prompt:** After computing the implement prompt, decide whether to generate a workflow prompt — a `/workflows`-compatible prompt that runs the implementation as a dynamic workflow with parallel subagents. **When `--workflow` is set**, always generate the workflow prompt regardless of plan complexity. **When `--workflow` is absent**, generate a workflow prompt only when the plan meets **any** of these criteria: (a) touches 5+ files across 3+ directories, (b) involves repetitive per-file changes (migrations, renames, linting sweeps), (c) includes independent steps that can run in parallel, or (d) requires cross-checking or adversarial review between steps. The workflow prompt format is: `Run a workflow to implement the plan at <filepath> — <objective>. Brief subagents with the plan file at <filepath>`, where `<filepath>` is the same relative path used in the implement prompt. Example: `Run a workflow to implement the plan at docs/plans/migrate-api-endpoints.html — Migrate all REST endpoints from v1 to v2 schema. Brief subagents with the plan file at docs/plans/migrate-api-endpoints.html`. Store this as `{workflow-prompt}` and fill the skeleton placeholder. When no workflow prompt is generated (flag absent and no criteria met), leave `{workflow-prompt}` empty and remove the workflow row from the `.plan-more-ways` drawer entirely — never leave an empty row. The drawer itself always stays and still works with the remaining goal and plan-source rows.
3. **Frontmatter** — Embed plan metadata as `<meta>` tags inside the HTML `<head>`, not as YAML. Include: `status` (`todo` | `in-progress` | `completed`), `effort` (the lowercase `{effort-value}` from Step 2 — `low` | `medium` | `high`, e.g. `<meta name="plan-effort" content="medium">`; never the capitalised display label), `type`, `created` (YYYY-MM-DD), `repo-name`, `file` (the plan basename from `{plan-filename}`), `path` (the relative plan path from `{plan-path}`), and `implement` (the full implement prompt computed in Step 2, e.g. `<meta name="plan-implement" content="Read and implement all steps in the plan at docs/plans/add-dark-mode-toggle.html — Ship a dark-mode toggle that persists across all three themes">`). **Always** include `goal` (the goal prompt computed in Step 2, e.g. `<meta name="plan-goal" content="Achieve this goal: Ship a dark-mode toggle that persists across all three themes. The plan at docs/plans/add-dark-mode-toggle.html describes one approach — use it as reference, but optimize for the outcome">`) — this tag is never omitted, since the goal prompt is always generated. When a workflow prompt was generated in Step 2, also include `<meta name="plan-workflow" content="Run a workflow to implement the plan at …">` — omit this tag entirely when no workflow prompt was generated. Resolve `repo-name` from the basename of the `origin` git remote URL (strip trailing `.git`); if no remote exists, fall back to the basename of the current working directory. If `--priority` was set, also add `<meta name="plan-priority" content="<level>">`. If an issue URL was fetched in Step 0.5, add `<meta name="plan-issue" content="<url>">` — omit this tag entirely when no issue reference was provided. Read `planAgent.extraFrontmatter` from `.claude/settings.json` (project first, then global) and render any extra key-value pairs as additional `<meta name="plan-<key>" content="<value>">` tags; `--priority` overrides any `priority` key from settings.
...
6. **Status** — **Always** update `status` in the HTML plan as the plan progresses: `todo` → `in-progress` → `completed`. Edit **both** the `<html data-status="…">` attribute and the `<meta name="plan-status" content="…">` tag so the CSS badge colour and the hook's completion check stay in sync. Also update the visible badge text. All completion state is carried by HTML attributes written into the file — step completion is the `.completed` class on each `.step-card`, and criterion completion is the `checked` attribute on each `#criteria-list` `<input>`. Mark a criterion by **adding** the attribute (`<input type="checkbox">` → `<input type="checkbox" checked>`) and unmark it by **removing** the attribute — an attribute edit, never a JS property toggle or browser-only persistence. These attributes are the single portable source of truth: they travel with the file across machines, browsers, and git. Note: `plan-interview:plan-status` operates on YAML-frontmatter `.md` files only — do not use it for HTML plans until that plugin is updated to support `.html`.
...
8. **Implement, Edit, or Exit** — After Step 7 completes, always ask the user what to do next.
   ...
   **Acceptance criteria gate (mandatory — runs after all steps are done, before marking `completed`):**
   ...
   3. Check off each criterion by **adding** the `checked` attribute (`<input type="checkbox">` → `<input type="checkbox" checked>`) only after confirming it is met; to undo a mark, **remove** the attribute. This is an attribute edit written into the file — the `checked` attribute is the portable source of truth, never `localStorage` or any browser-only persistence.
   ...
   6. This gate edits checkbox state and status only.
...
- **at-a-glance** — A 2–3 sentence plain-language summary filling the `{at-a-glance}` placeholder in the `.plan-glance` block, rendered as a **sibling immediately after `div#objective`** (never a child of it). Job split with the objective: objective = the one-line *what*; at-a-glance = *why it matters* and *how we'll know it worked*. They must never restate each other. See the At a glance rule in Step 2.
...
- **acceptance-criteria** — Rendered under the literal heading **"Definition of done"** (the `#criteria` section's `<h2>` text; the section id and `#criteria-list` are unchanged). Interactive checkboxes (`<input type="checkbox">`) the user can tick off in the browser. Each item is a short, falsifiable statement.
- **verification** — Rendered under the literal heading **"Final check"** (the `#verification` section's `<h2>` text; the section id is unchanged). How to confirm the entire plan was executed correctly end-to-end.
...
- **resources** *(optional)* — Images, screenshots, and reference links to the material used to create the plan, so the reader can illustrate and verify the implementation. Rendered as `section.card-resources#resources` (photo icon `#ic-photo`), between Context and Files. Include when the plan was informed by screenshots, mockups, diagrams, external docs, or issues worth citing; delete the whole block **and** its sidebar nav link (the `#resources` link) when the plan has no supporting resources. See [Resources Capture](#resources-capture).
...
- Include an effort badge in the page header (colour-coded: green = low, amber = medium, red = high) driven by the `<html data-effort="…">` attribute, plus an effort chip in the `.plan-meta` row. The effort level is auto-derived in Step 2 and always present — never omitted, no extra JavaScript
...
- Be readable without JavaScript (checkboxes work natively); minimal JS is allowed only to enhance interactivity (e.g. progress bar update on checkbox change). The `checked` attribute on each criterion `<input>` (and the `.completed` class on each `.step-card`) is the **portable source of truth** — written into the file, it renders correctly on first paint on any machine, in any browser, and in git. **Never** use `localStorage` or any browser-only persistence for checkbox state; it is per-browser, never written back to the file, and would diverge from the on-disk attributes
- Include `<meta name="plan-status">`, `<meta name="plan-effort">`, `<meta name="plan-type">`, `<meta name="plan-created">`, `<meta name="plan-repo">`, `<meta name="plan-file">`, `<meta name="plan-path">`, `<meta name="plan-implement">`, and `<meta name="plan-goal">` tags for machine readability; include `<meta name="plan-workflow" content="…">` when a workflow prompt was generated (omit otherwise); include `<meta name="plan-issue" content="<url>">` when the plan was seeded from an issue reference (omit otherwise)
...
- Include an **At a glance block** (the `.plan-glance` element) as a **sibling immediately after `div#objective`** and before the `.plan-implement` row — never nested inside `#objective` (the extractor reads `#objective`'s inner HTML). Fill its `{at-a-glance}` placeholder with the 2–3 sentence plain-language summary written in Step 2. Keep the block's visible "At a glance" label and its accessible-name wiring (`aria-labelledby` / `aria-label`) exactly as the skeleton ships them.
- Include an **implement prompt row** (`<div class="plan-implement">`) immediately below the `.plan-glance` block and before `.progress-wrap`. It must contain: the `{implement-prompt}` value in `<code id="implement-cmd" aria-label="Implement prompt">`, and a Copy button with `onclick="copyCmd(this)"`. The Copy button copies a concise action-oriented prompt (built by `buildImplementPrompt()` from live DOM state including status, progress counts, and implementation instructions) — not the short `<code>` text. The copy button is hidden via CSS when `data-status="completed"`. The row is suppressed in `@media print`. **The Implement row is the single visible prompt call-to-action on first paint** — every other prompt row lives inside the `.plan-more-ways` drawer below.
- Include a **More-ways drawer** (`<details class="plan-more-ways">`) immediately below the `.plan-implement` row and before `.progress-wrap`, with its `<summary>` reading "More ways to run this plan". It is **one flat `<details>`**: collapsed by default (never ship the `open` attribute) and containing **no nested `<details>` elements** — the rows inside are plain elements, not disclosures. It holds, in order, the goal row, the workflow row (when generated), and the plan-source rows described below. Each row keeps its distinguishing label text and its existing `code` id and copy handler unchanged.
- Include an always-present **goal row** inside the `.plan-more-ways` drawer. Its label reads "Pursue as goal — optimize for the outcome" — the leading `Pursue as goal` text is a frozen string (see [Frozen Strings](#frozen-strings)). The row contains the `{goal-prompt}` value in `<code id="goal-cmd">` and a Copy button with `onclick="copyGoal(this)"`. Hidden when `data-status="completed"` and suppressed in print. Unlike the workflow row, it is **never removed** — every plan gets a goal prompt.
- Include the **plan source rows** inside the `.plan-more-ways` drawer, after the goal and workflow rows — a `File` row with the basename in `<code id="plan-file">` and a `Path` row with the relative path in `<code id="plan-path">`, each followed by a Copy button with `onclick="copyPath(this, 'plan-file')"` / `onclick="copyPath(this, 'plan-path')"`. Fill `{plan-filename}` and `{plan-path}` (HTML-escaped). This gives the user the plan's name and relative URL to paste into docs and prompts. Unlike the goal and workflow rows, these stay visible when `data-status="completed"`; they are suppressed in `@media print`.
- When a workflow prompt was generated, include a **workflow row** inside the `.plan-more-ways` drawer, between the goal row and the plan-source rows. Its label reads "Run as workflow — launch parallel subagents". It contains the `{workflow-prompt}` value in `<code id="workflow-cmd">` and a Copy button with `onclick="copyWorkflow(this)"`. Hidden when `data-status="completed"` and suppressed in print. **When no workflow prompt was generated, omit the workflow row entirely** — never leave an empty row, and never remove the drawer itself: it still renders and works with the goal and plan-source rows.
- Include a **completion checklist** section (`<section class="section-card card-completion" id="completion">`) between Verification and Next Steps with three `disabled` checkboxes for the mandatory completion requirements (step TODOs done, acceptance criteria checked, status updated). The checkboxes auto-update via JavaScript. Include a **Completion Report** (`<div class="completion-report">`) inside the checklist that initially shows the frozen sentence "No items to report — all requirements met." (see [Frozen Strings](#frozen-strings)) and is populated with a `<dl class="report-list">` detailing any incomplete items and their reasons when the plan cannot be fully completed.
...
- **Resources section.** Render a `<section class="section-card card-resources" id="resources">` between Context and Files **only** when the plan was informed by screenshots, mockups, diagrams, external docs, or issues worth citing (see [Resources Capture](#resources-capture)). Fill `{resource-figures}` with `.resource-figure` cards (each a `<figure>` with an `<img>` — mandatory descriptive `alt` — and a `<figcaption>` that credits/links the source) and `{resource-links}` with `.resource-links` list items (each an `<a>` plus a `.resource-note`). Images use an `https://` source URL or a relative asset path committed alongside the plan (small screenshots may be inlined as a `data:` URI); never rely on a CDN for anything the page needs to render. Remove either sub-block (figures or links) that has no content, and delete the whole section **and** its sidebar nav `<li>` (the `#resources` link) when there are no resources. HTML-escape every URL, caption, and note.

### Frozen Strings

Downstream consumers match these strings **byte-for-byte**. Reproduce them exactly as the skeleton ships them — never reword, re-case, re-punctuate, or swap the em dash for a hyphen. Humanization applies to everything *around* these strings, never to the strings themselves:

- `<span class="step-chip">todo</span>` — the initial chip on every step card. The `finalize-plan` skill marks steps complete via a **literal find/replace** of this exact markup to `<span class="step-chip">done</span>`; any variation makes finalization silently no-op. `todo` and `done` are the only two legal chip tokens.
- `No items to report — all requirements met.` — the initial `report-empty` sentence inside the Completion Report. `finalize-plan` and the plugin smoke tests expect this exact sentence verbatim.
- `Pursue as goal` — the leading text of the goal row's label ("Pursue as goal — optimize for the outcome"). `tests/plugins/test-goal-prompt.sh` greps this literal label.
...
| **Resources** | was informed by screenshots, mockups, diagrams, external docs, or issues worth citing | `.resource-grid` → `.resource-figure` (image + caption); `.resource-links` → `<li>` (link + `.resource-note`) | `{resource-figures}`, `{resource-links}` — see [Resources Capture](#resources-capture) |
...
- **Other visuals are opt-in.** Keep a block only when its trigger matches; delete the whole block and its sidebar nav link otherwise — exactly as you already do for the workflow row and unresolved-questions.
- **Self-contained.** Pure CSS / inline SVG only. No CDN, no `<canvas>`, no charting library.
- **Accessible.** Tables: `<caption>` + scoped headers. Charts: visible numeric values + a descriptive container `aria-label` (never colour-only). File badges: a text label alongside the colour. Resource images: mandatory descriptive `alt` text.
- **HTML-escape** all file paths, code, and user-supplied text inside these blocks (`&` → `&amp;`, `<` → `&lt;`, `>` → `&gt;`).

### Resources Capture

When the plan is informed by material worth showing — a screenshot of current behaviour, a design mockup, an architecture diagram, an external doc, a reference implementation, or the seeding issue — capture it in the **Resources** section so the reader can *illustrate* what the plan is about and *verify* the implementation against the same references. This is opt-in: a plan drafted from a plain objective with no visual or external references simply omits the section.

**When to capture.** During Step 0b Explore and Step 1 Clarify, keep a running list of the references you actually consult:

- **Web research** (`WebSearch` / `WebFetch`) — the docs, API references, changelogs, or best-practice articles you read to shape the plan. Cite each as a **link** with a short note on what it informed.
- **Screenshots** — a browser screenshot of the current (before) state, an error you are fixing, or a rendered page the plan changes. Capture as an **image**.
- **Mockups / diagrams** — a design mockup, wireframe, Figma frame, or architecture diagram the plan implements. Capture as an **image** (link the editable source in the caption).
- **Seeding issue** — when the plan came from a `#n` / issue URL (Step 0.5), add the issue as a **link**.

**Images.**

- Prefer an `https://` source URL for material that lives on the web (doc diagrams, hosted screenshots). For a local screenshot captured during planning, save it into an `assets/` (or `images/`) subdirectory beside the plan and reference it by **relative path**, or inline a **small** screenshot as a `data:` URI — either way it travels with the plan in git. **Never rely on a CDN** for anything the page needs to render (the no-CDN, self-contained constraint still applies).
- Every `<img>` needs **descriptive `alt` text** stating what the image shows — not "screenshot".
- Wrap the `<img>` in a link to the full-resolution source when one exists, and use `<figcaption>` to say what it shows and credit/link the origin.

**Links.** One `<li>` per reference: an `<a href>` (with `target="_blank" rel="noopener noreferrer"`) and a `.resource-note` explaining why it matters to this plan. Drop bare URLs — always give the link a human title.

**Rendering.** Fill `{resource-figures}` inside `.resource-grid` and `{resource-links}` inside `.resource-links`, following the templates in `SKELETON.html`. Remove whichever sub-block (figures or links) is empty. HTML-escape every URL, caption, and note. If no resources were consulted, delete the entire `section.card-resources#resources` block **and** its sidebar nav `<li>` (the `#resources` link) — the scroll-spy filters to existing sections, so removal needs no JS edits.
...
**Plain language:** Write every reader-facing sentence for a reader who wasn't in the planning session — assume no memory of the conversation that produced the plan. Expand jargon, acronyms, and project-specific shorthand on first use (*"the scroll-spy (the sidebar highlighting that tracks your scroll position)"*, *"WCAG (the web accessibility guidelines)"*); after the first expansion, the short form is fine. This applies to the objective, the at-a-glance summary, section intros, step actions and whys, acceptance criteria, and verification text alike. Precision stays — real file paths, function names, and CLI flags are never dumbed down; it is the connective prose around them that must read as plain English.

**Section intros:** the skeleton places a one-line `<p class="section-intro">` directly under each section heading — a single plain-language sentence telling the reader what the section is for (e.g. *"The step-by-step work, in order — each step says what to do, why it matters, and how to check it worked."*). Fill or adapt each intro as one sentence; keep it a `<p>` (never a heading element, never `var(--subtle)` styling), and never move intro copy inside the `#objective`, `#context`, or `#verification` body text that the plan-spec extractor (`scripts/lib/plan-spec.mjs`) extracts.
...
Each step card in the skeleton includes a `<span class="step-chip">todo</span>` before the action text. Always write `todo` as the initial chip value — the chip visually updates to `done` via CSS when the `.step-card.completed` class is applied. Do not change the chip text from `todo` in the initial HTML output — this markup is a frozen string (see [Frozen Strings](#frozen-strings)).
...
The skeleton also ships visual blocks behind removal comments, just like the workflow row inside the `.plan-more-ways` drawer. The **files** block is **auto-generated** from the plan's steps (see [File-Tree Auto-Generation](#file-tree-auto-generation)) — always keep it when any files are referenced, delete it only for purely conceptual plans with zero file references. The **resources** block (see [Resources Capture](#resources-capture)) and the **diagram**/**chart**/**table** blocks remain opt-in: keep and fill them when the plan's content warrants the visual (see **Visual Components** for triggers), otherwise delete the entire block **and** its sidebar nav `<li>` (the `#resources` or `#diagram` link). All CSS stays in the skeleton's `<style>` either way — it is inert when the markup is absent.
