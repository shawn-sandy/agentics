...
```
   ...
   **Effort level:** After drafting all steps (and the file-tree), compute an effort level — an auto-derived estimate of how much work the plan represents, surfaced as a colour-coded badge, a header meta-row chip, and a `plan-effort` meta tag. It is **always generated** — no flag, no author input. Derive it deterministically from plan size: weigh the **step count**, the number of **distinct files** in the file-tree, and the **Step 5b interview complexity tier** (short / medium / complex), then map to one of three buckets:
   ...
   When Step 5b was skipped (`--quick` / `--no-interview`), derive from the step and file counts alone. Store the capitalised label (`Low` | `Medium` | `High`) as `{effort}` for display, and the lowercase form (`low` | `medium` | `high`) as the `data-effort` attribute value on `<html>`. Fill every `{effort}` placeholder and the `data-effort` attribute in the skeleton.
...
- Include an always-present **goal prompt** (`<details class="plan-goal">`) immediately below the `.plan-implement` row. The `<summary>` reads "Pursue as goal — optimize for the outcome". The inner `<div class="plan-goal-inner">` contains the `{goal-prompt}` value in `<code id="goal-cmd">` and a Copy button with `onclick="copyGoal(this)"`. Collapsed by default. Hidden when `data-status="completed"` and suppressed in print. Unlike `.plan-workflow`, it is **never removed** — every plan gets a goal prompt.
