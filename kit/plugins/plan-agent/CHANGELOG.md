...
- **Backfill script** — `scripts/backfill-plan-digests.mjs` injects digests into existing `docs/plans/*.html` (idempotent, insertion-only, `--dry-run`, `--dir`). Plans that cannot be fully parsed are skipped and reported — no partial digests. Backfilled 40 of 52 plans; the 11 reported skips are 8 `*-review.html` artifacts and 3 pre-skeleton-era plans, plus 1 plan that already carried a digest.
