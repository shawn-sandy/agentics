...
- **Section headings and intros humanized** — `reference/SKELETON.html` headings moved to sentence case with a one-line intro under each, and the markdown fallback `reference/SKELETON.md` was mirrored to the same structure so both output formats read the same.
