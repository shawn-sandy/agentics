...
- `Exit — I'll implement later` and `Run as workflow` at the chained Step 8 both terminate the **outer** chain. Step 8 is the only point at which the user is asked how to execute, so treating either as declining merely the inner offer would build work they just routed elsewhere.
