...
description: >
  Background plan-review agent. Runs the ten-reviewer Workflow
  ...
  plan without blocking the parent session. Improves and updates the plan in
  place. Use when delegating plan review to a subagent so the main session can
  keep working.
  Mirrors the review-plan skill but runs as a background subagent.
tools: Skill, Read, Write, Edit, Glob, Grep, Bash, Artifact, Workflow
model: opus
maxTurns: 30
background: true
---
