# Blog Share — Platform Formatting Rules

Read this file in Phase 3 to draft platform-aware copy. Follow the format for the
selected platform exactly — character limits are hard constraints, not suggestions.
...
1. [First key takeaway from the article — one to two sentences]
...
```
[One punchy sentence that names the single most surprising or useful insight from
the article — make someone want to click.] [URL] #[Tag] #[Tag]
```

**Rules:**
- No "Great article by…" opener — lead with the insight
- URL counts as 23 characters (t.co shortener)
...
Worth your time if you [relevant condition / audience]. [URL]
```
...
- No hashtags required (lower hashtag culture than Twitter)
