...
## v2.23.1 — 2026-08-17 — screenshots verify themselves before delivery
...
- **`templates/gallery.html` takes the plan-agent 9.1.0 palette.** The media
  library carries its own copy of the shared token set, so when plan-agent
  retuned the palette this gallery was left as the odd page out — warm cream
  `#fcfcfa` and a violet `#4a2fe0` accent behind a top nav whose other three
  tabs had all moved to cool neutrals and indigo. Same token names, new values:
  neutrals are faintly indigo-biased, `--accent` is `#3730c4` / `#a8a2f5`, and
  the moss and signal states are desaturated to match.
- **`--prose` resolves to `var(--ui)`.** It was Georgia, which is the same
  serif/mono collision the plan document just shed.

`docs/media/social/index.html` is generated by the `media-library` skill rather
than a script, so the committed gallery was updated in place with the identical
substitution. Regenerating it produces the same result.
