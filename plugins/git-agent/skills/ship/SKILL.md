...
name: ship
description: >
  Use when the user asks to ship changes, commit and create a PR or MR, commit
  push and PR, ship it, send it, or land their work. Chains commit + push +
  pull/merge request into one flow. Supports GitHub (gh) and GitLab (glab).
  Use commit-agent for commit only or pr-agent for PR only.
---

Stage, commit, push, and create a pull/merge request in one flow. Supports GitHub (`gh`) and GitLab (`glab`). Follow these steps in strict order. **STOP immediately after step 8.**
...
**Clean working tree:**
Run `git status`. If nothing to commit, output: "Nothing to ship — working tree is clean." and **STOP**.

**Detached HEAD:**
Run `git branch --show-current`. If the output is empty, output: "Cannot ship: repository is in detached HEAD state. Checkout a branch first." and **STOP**.

**On main or master:**
If the current branch is `main` or `master`, output: "Cannot ship from the default branch. Switch to a feature branch first." and **STOP**.
...
- If neither can be determined, ask the user which platform they use and **STOP**.

**CLI not available or not authenticated:**

For GitHub: run `gh auth status`. If `gh` is not installed or returns an auth error, output:
```
GitHub CLI is required. Install it from https://cli.github.com/ and run `gh auth login`.
```
and **STOP**.

For GitLab: run `glab auth status`. If `glab` is not installed or returns an auth error, output:
```
